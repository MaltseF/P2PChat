    ...
    public partial class MainWindow
    {
        ...
        private void Window_Loaded(object sender, RoutedEventArgs e)
        {
            ...
            foreach (IPAddress address in Dns.GetHostAddresses(Dns.GetHostName()))
            {
                ...
                {
                    serviceUrl = $"net.tcp://{address}:{port}/P2PService";
                    break;
                }
            }
            ...
            host = new ServiceHost(localService, new Uri(serviceUrl ?? throw new InvalidOperationException()));
            ...
            peerNameRegistration = new PeerNameRegistration(peerName, int.Parse(port)) {Cloud = Cloud.AllLinkLocal};
        ...
        private void PeerList_Click(object sender, RoutedEventArgs e)
        {
            ...
            if (((Button) e.OriginalSource).Name == "MessageButton")
            {
                ...
                if (peerEntry?.ServiceProxy != null)
                {
                    try
                    {
                        peerEntry.ServiceProxy.SendMessage(Crypt.Encrypt(peerEntry.Message, "huyahu"),
                            ConfigurationManager.AppSettings["username"]);
                        fldMessages.Text += $"Вы: {peerEntry.Message} \n";
                    }
        ...
        internal void DisplayMessage(string message, string from)
        {
            ...
            //MessageBox.Show(this, message, $"Сообщение от {from}",
            //    MessageBoxButton.OK, MessageBoxImage.Information);

            fldMessages.Text += $"Собеседник ({from}): {Crypt.Decrypt(message, "huyahu")} \n";
        }
    }
}
